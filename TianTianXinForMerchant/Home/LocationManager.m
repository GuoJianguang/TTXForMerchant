...
//    NSLog(@"---%f",manager.location.coordinate.latitude);
    [TTXUserInfo shareUserInfos].locationCoordinate = manager.location.coordinate;
    // 已经不需要再定位了
    ...
    [geoCoder reverseGeocodeLocation:[locations lastObject] completionHandler:^(NSArray *placemarks, NSError *error) {
        ...
        if(city) {
            ...
            if (self.finishLocation) {
                self.finishLocation(nil,city,self.areaName,nil,YES);
            }
        }
        else {
            if (self.finishLocation) {
...
//    [[UIAlertViewHelper shareAlterHelper]showTint:@"定位失败,您可以手动选择城市" duration:1.5];
    ...
    if (self.finishLocation) {
        self.finishLocation(nil,nil,@"",nil,NO);
    }
}
...
- (void)startLocationWithGDManager
{
    ...
    [self.gdLocationManager requestLocationWithReGeocode:YES completionBlock:^(CLLocation *location, AMapLocationReGeocode *regeocode, NSError *error) {
        ...
        if (error)
        {
            ...
            {
                self.finishLocation(regeocode,@"成都市",regeocode.district,nil,YES);
        ...
        if (regeocode)
        {
            ...
            self.finishLocation(regeocode,regeocode.city,regeocode.district,nil,YES);
